    ...
    public class OrderDAL : _BaseDAL, IOrderDAL
    {
        ...
        public int Count(int status = 0, DateTime? fromTime = null, DateTime? toTime = null, string searchValue = "")
        {
            ...
            using (var connection = OpenConnection())
            {
                ...
                {
                    Status = status,
                    FromTime = fromTime ?? DateTime.Now,
                    ToTime = toTime ?? DateTime.Now,
        ...
        public Order? Get(int orderID)
        {
            ...
            using (var connection = OpenConnection())
            {
                var sql = @"select  o.*, c.CustomerName, c.ContactName as CustomerContactName, c.Address as CustomerAddress, c.Phone as CustomerPhone,
